jQuery.fn.extend({
  ...
  _initNumDom: function(opt){
    for (var i = 0,len = this.length; i < len; i++) {
      ...
      var inputN = $('<input type="text" value="'+opt.min+'" class="'+opt.inputClass+'"/>');
  ...
  _initNumEvent: function(it, ab, sb, opt){
    ...
    it.on('keyup',function(){
    	floatCount=opt.floatCount-1;
      var val = $(this).val();
      if((opt.positive&&opt.negative)||(!opt.positive&&!opt.negative)){
        var reg = new RegExp('^\D*(-?(([0-9]\\d*)?|([0]))?(\\.\\d{0,'+floatCount+'})?)?.*$', 'g');
      }else if(!opt.positive&&opt.negative){
        var reg = new RegExp('^(-(([0-9]\\d*)?|([0]))?(\\.\\d{0,'+floatCount+'})?)?.*$', 'g');
      }else if(opt.positive&&!opt.negative){
        var reg = new RegExp('^\D*(([0-9]\d*)?|([0]))?(\.\d{0,'+floatCount+'})?)?.*$', 'g');
      }
